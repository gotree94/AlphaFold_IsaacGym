# RTX 5090 24GB 로컬 LLM 완벽 가이드

## 목차
1. [주요 LLM 프레임워크](#1-주요-llm-프레임워크)
2. [실행 가능한 모델](#2-rtx-5090-24gb에서-실행-가능한-모델)
3. [최적 구성](#3-rtx-5090-24gb-최적-구성)
4. [성능 비교](#4-성능-비교-rtx-5090-24gb)
5. [특수 용도별 추천](#5-특수-용도별-추천)
6. [설치 용량](#6-설치-용량)
7. [최종 추천 구성](#7-최종-추천-구성)
8. [통합 환경](#8-통합-환경-구성)
9. [고급 기법](#9-ram--vram-통합-사용)
10. [비용 및 라이선스](#10-비용-및-라이선스)
...
| 모델 크기 | 양자화 | VRAM 사용 | 속도 | 품질 |
|-----------|--------|-----------|------|------|
| **7B** | FP16 | 14GB | 매우 빠름 | 좋음 |
| **7B** | Q4_K_M | 4GB | 초고속 | 양호 |
| **13B** | FP16 | 26GB | ❌ 불가 | - |
| **13B** | Q4_K_M | 8GB | 빠름 | 좋음 |
| **34B** | Q4_K_M | 20GB | 보통 | 매우 좋음 |
| **70B** | Q4_K_M | 40GB | ❌ 불가 | - |
| **70B** | Q3_K_M | 27GB | ❌ 불가 | - |
| **70B** | Q2_K | 20GB | 보통 | 양호 |
...
| 모델 | 양자화 | VRAM | 속도 | 용도 |
|------|--------|------|------|------|
| Llama 3.2 3B | Q4 | 2GB | ~150 tok/s | 채팅, 빠른 응답 |
| Llama 3.1 8B | FP16 | 14GB | ~80 tok/s | 범용 |
| Mistral 7B | Q4 | 4GB | ~100 tok/s | 범용 |
| Qwen 2.5 14B | Q4 | 8GB | ~60 tok/s | 추론, 다국어 |
| DeepSeek Coder 16B | FP16 | 18GB | ~50 tok/s | 코딩 |
| Qwen 2.5 32B | Q4 | 20GB | ~30 tok/s | 고품질 추론 |
| CodeLlama 34B | Q4 | 20GB | ~25 tok/s | 코딩 |
...
```bash
conda create -n biomedical-llm python=3.10 -y
...
./start_linux.sh
```
**이유**: GUI 선호, 여러 모델 테스트, ChatGPT 스타일

---

## 요약

✅ **RTX 5090 24GB로 실행 가능**
- 7B-34B 모델: 완벽 실행
- 70B 모델: Q2 양자화 또는 오프로딩 필요
- 추천: Qwen 2.5 32B (최고 품질/성능 균형)
...
**RTX 5090 24GB는 대부분의 LLM을 로컬에서 실행하기에 충분한 성능입니다!** 🚀
